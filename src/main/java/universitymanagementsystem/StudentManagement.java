...
public class StudentManagement extends Application {
    ...
    public class FacultyController {
        ...
        public FacultyController(ObservableList<Student> students) {
            this.students = students;
        }

        public ObservableList<Student> getStudentsInCourse(String courseCode) {
    ...
    public class FacultyUI {
        ...
        public FacultyUI() {
            this.controller = new FacultyController(students);
        }

        public VBox createFacultyDashboard() {
            VBox root = new VBox(10);
            root.setPadding(new Insets(15));

            Label titleLabel = new Label("Faculty Dashboard");
            titleLabel.setStyle("-fx-font-size: 20; -fx-font-weight: bold;");

            TextField courseField = new TextField();
            courseField.setPromptText("Enter Course Code");
            ...
            TableColumn<Student, String> levelCol = new TableColumn<>("Academic Level");
            levelCol.setCellValueFactory(new PropertyValueFactory<>("academicLevel"));

            studentTable.getColumns().addAll(idCol, nameCol, levelCol);

            searchButton.setOnAction(e -> {
                String courseCode = courseField.getText().trim();
                if (!courseCode.isEmpty()) {
                    studentTable.setItems(controller.getStudentsInCourse(courseCode));
                } else {
                    showAlert("Error", "Please enter a course code");
                }
            });

            addReturnButton(root);
            root.getChildren().addAll(titleLabel, courseField, searchButton, studentTable);
            return root;
        }
    ...
    public StudentManagement() { this(false, false); }
    ...
    public void start(Stage stage) {
        ...
        if (isAdmin) {
            ...
            VBox root = facultyUI.createFacultyDashboard();
